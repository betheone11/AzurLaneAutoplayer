...
def select_team(flag_1, flag_2, index):
    ...
    if flag_1 == flag_2:
        ...
        if flag_2 == 2:
            ...
            clip_select_team(2)
            utils.cf_action(Contains.src, flag[flag_2], index)
    elif flag_1 == 2:
        clip_select_team(2)
        utils.cf_action(Contains.src, flag[6], index)
        clip_select_team(1)
        utils.cf_action(Contains.src, flag[flag_1], index)
        if flag_2 == 6:
            pass
        else:
            clip_select_team(2)
...
def gaming_retire(index):
    ...
    while True:
        ...
        if utils.find_image(Contains.src, Contains.retire):
            ...
            yes(index)
            time.sleep(random.uniform(0.5, 1.5))
            utils.tap(round(random.uniform(100, 1000), 3),
                      round(random.choice([random.uniform(100, 282), random.uniform(510, 700)]), 3), 1)
            time.sleep(random.uniform(0.5, 1.5))
            yes(index)
...
def clear_storehouse(index):
    ...
    try:
        ...
        if index == 0:
            utils.cf_action(Contains.src, Contains.autoplay, index)
    except:
        # 添加自定义异常
        time.sleep(2)
        utils.cf_action(Contains.src, Contains.autoplay, index)
...
def low_mood(index):
    # 地心情是退出作战
    while_cancel(index)
    utils.cf_action(Contains.src, Contains.Quit, index)
    yes(index)
